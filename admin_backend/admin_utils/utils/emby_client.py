...
class EmbyClient:
        ...
        try:
            ...
            if result:
                ...
                if password and user_id:
                    self._request('POST', f'/Users/{user_id}/Password', json={
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        Returns:
            是否成功
        """
        result = self._request('POST', f'/Sessions/{session_id}/Logout')
        return result is not None

    def get_active_transcodings(self) -> List[Dict]:
        """
        获取当前活跃的转码任务
        ...
        for session in sessions:
            # 检查是否有转码信息
            now_playing = session.get('now_playing_item')
            play_state = session.get('play_state', {})

            if play_state.get('PlayMethod') == 'Transcode':
                transcodings.append({
                    ...
                    'item_type': now_playing.get('Type') if now_playing else 'Unknown',
                    'series_name': now_playing.get('SeriesName') if now_playing else None,
                    'container': play_state.get('Container'),
                    'video_codec': play_state.get('VideoCodec'),
                    'audio_codec': play_state.get('AudioCodec'),
                    'bitrate': play_state.get('Bitrate', 0),
                    'framerate': play_state.get('Framerate'),
                    'width': play_state.get('Width', 0),
                    'height': play_state.get('Height', 0),
                    'audio_channels': play_state.get('AudioChannels', 0),
                    'transcode_reasons': play_state.get('TranscodeReasons', []),
                    'is_video': play_state.get('IsVideo', False),
                    'progress': play_state.get('PositionTicks', 0) / now_playing.get('RunTimeTicks', 1) * 100 if now_playing else 0,
                })
        return transcodings
            ...
            return [
                {
                    ...
                    'primary_image_item_id': lib.get('PrimaryImageItemId'),
                }
    ...
    def refresh_library(self, library_id: str = None) -> bool:
        ...
        if library_id:
            # 刷新指定媒体库
            result = self._request('POST', f'/Items/{library_id}/Refresh', json={
                'Recursive': True,
            ...
            for lib in libraries:
                self._request('POST', f'/Items/{lib["id"]}/Refresh', json={
                    'Recursive': True,
                    'ImageRefreshMode': 'FullRefresh',
                    'MetadataRefreshMode': 'FullRefresh',
                    'ReplaceAllImages': False,
    ...
    def scan_library(self, library_id: str = None) -> bool:
        """
        扫描媒体库（不刷新元数据，只添加新内容）

        Args:
            library_id: 媒体库 ID，为空则扫描所有
    ...
    def get_system_info(self) -> Dict:
        ...
        Returns:
            系统信息
        """
        return self._request('GET', '/System/Info') or {}

    def get_activity_log(self, start_index: int = 0, limit: int = 50) -> List[Dict]:
        """
        获取活动日志
        ...
        Returns:
            活动日志列表
        """
        logs = self._request('GET', f'/System/ActivityLog/Entries?StartIndex={start_index}&Limit={limit}')
        if logs:
            return [
                {
                    'id': log.get('Id'),
                    ...
                    'user_primary_image_tag': log.get('UserPrimaryImageTag'),
                    'overview': log.get('Overview'),
                    'short_overview': log.get('ShortOverview'),
                    'item_id': log.get('ItemId'),
                    'item_name': log.get('ItemName'),
                    'item_type': log.get('ItemType'),
                    'season_id': log.get('SeasonId'),
                    'series_name': log.get('SeriesName'),
                }
