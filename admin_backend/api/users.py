...
async def get_user_stats(
    ...
    """获取用户统计数据（从 PostgreSQL）"""
    # 直接使用 PostgreSQL 的 web_users 表
    from sqlalchemy import text

    # 总用户数
    total_users = db.execute(text("SELECT COUNT(*) FROM web_users")).scalar() or 0

    # VIP 用户数（有有效订阅的用户）
    vip_users = db.execute(text("""
    ...
    new_users_today = db.execute(text("""
    ...
    active_users_week = db.execute(text("""
        SELECT COUNT(*) FROM web_users
        WHERE created_at >= :week_ago
    """), {"week_ago": week_ago}).scalar() or 0

    # 订单总数
    total_orders = db.execute(text("SELECT COUNT(*) FROM subscription_orders")).scalar() or 0
