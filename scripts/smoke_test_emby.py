...
assert r.status_code == 200, r.text
server_info = r.json()
# 查看权限 gating：alice 无订阅、默认服为付费服 → 敏感字段必须置空不下发
...
db3.add(plan)
db3.flush()
db3.add(models.UserSubscription(user_id=alice_id, plan_id=plan.id, realm_id=realm_id,
                                status="active",
