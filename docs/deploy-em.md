...
**免登（单点登录）**：门户与后台同源、共用同一套 JWT，管理员在门户登录后，后台会用该会话静默验证一次并直接进入，**不需要再输一遍账号密码**（顶栏用户菜单与移动端抽屉里有「管理后台」入口）。门户 token 过期时会自动用 refresh token 换新；门户未登录或账号不是管理员时，后台才显示登录页并说明原因。
