...
登录管理后台后打开「系统 → Emby 服务入口」。这里有两个完全不同的选择：

- **分离部署后端服（EA）**：填 `serve_emby.py` 部署后的公网地址。EA 必须和 EM 使用同一个 `DATABASE_URL` 与 `SECRET_KEY`，先在 EA 服务器启动成功，再点击「测试连接」和「保存并使用」。测试不通过时，媒体库、扫描、挂载、会话和播放相关的自建功能会被明确拦截，不会出现“页面能点但实际不能播”的假状态。
- **已有 Emby 服**：填已经运行的 Emby 地址，可选填管理员 API Key 做连通性测试。这个模式只保存连接信息，不会把本项目的会员、用户、媒体库扫描和自建网关强行写进第三方 Emby；本项目的自建媒体库功能会停用。

保存分离部署后端服（EA）时，面板还会顺手向 EA 拉一次**EA 视角的挂载体检**：`local` / `strm` 的路径、rclone 的 RC 地址这类配置是「跟着服务器走」的，后台（EM）测试通过不代表那台 EA 能播。结果会显示在「存储挂载」页的「EM / EA 可达」列，被媒体库引用却在 EA 上不可达的挂载会单独报红（详见 `docs/operations.md`）。
