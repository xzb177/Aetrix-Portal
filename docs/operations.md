...
- **403 且提示「当前客户端已被停用」或「不在允许列表内」**：客户端准入生效（v2.26.0）。
  去「用户与账号 → 客户端策略」看黑 / 白名单：白名单一旦非空就是**只放行列表内的客户端**，
  连脚本与未知 UA 也会被拒（管理员不受限，可先用自己的账号验证是不是策略引起）；
- **403 且提示「本站已关闭服务端转码」**：策略里的「允许服务端转码」被关掉了；
  客户端与网页播放器只能直连，遇到浏览器不支持的编码就放不了，重新打开即可；
- **503 且提示「转码路数已达上限」**：并发的转码请求超过了策略上限（正在看的人不受影响，
  只是新请求被拒），要么等一会儿，要么把上限调大（先看页面上的运行态：本机几路 / 上限多少）；
...
### 管理员角色与权限（v2.26.0）：谁能改什么

后台账号就是前台账号（同一套 token 与密码），管理员身份仍然是 `is_staff`；在此之上多一层
**角色**（`web_users.admin_role`）：

| 角色 | 日常运营（用户 / 订阅 / 订单 / 卡码 / 求片 / 工单 / 媒体库 / 存储来源 / 扫描与体检） | 系统与经济设置 · 能力中心（上游 Key）· 播放与客户端策略 | 管理员与权限 |
| --- | --- | --- | --- |
| `super`（超级管理员） | ✅ | ✅ | ✅ |
| `operator`（运营） | ✅ | ❌ | ❌ |
| `viewer`（只读审计） | ❌（能看，不能改） | ❌ | ❌ |
...
- **判定只在服务端一处**（`get_current_admin` / `require_staff`）：界面上的置灰只是提前告知，
  直接用 `curl` 打接口同样会被 403；**扫描 / 体检 / 改媒体库都算写操作**，`viewer` 一样拦。
  Emby 兼容面上的「触发全库重扫 / 踢别人的播放会话 / 释放全部转码」与
  `/api/notifications/send|broadcast`（全站广播）也走同一处判定——侧门不算例外。
- **升级不变权**：`admin_role` 为空（老库补列后为 NULL）一律按 `super` 处理，
  升级前后权限完全一致，也不会出现「没人进得去后台」。
- **三条护栏**：不能改自己（降自己或撤销自己 = 把自己关在门外，只能改库）、
  最后一名超级管理员不能降级 / 撤销（得先给别人授予 super）、
  被停用的账号（`is_active=false`）不能授予管理员（那种号永远登不进来，先在「用户」页启用）。
- 授权 / 改角色 / 撤销 / 启用停用都进「操作日志」（`admin_grant` / `admin_update` / `admin_revoke`）。
- 被拦时错误信息直接告诉你缺什么：`这一项需要「超级管理员」角色（当前：运营）` /
  `当前账号是「只读审计」角色：可以查看，不能修改…`——按提示让超级管理员调角色即可。

### 播放与客户端策略（v2.26.0）：客户端到底允许做什么

以前这些策略分在两处：转码与码率是环境变量（`EMBY_MAX_TRANSCODES` / `EMBY_TRANSCODE_IDLE`，
改一次要登机器重启进程），下载与设备开关在系统设置里。现在统一在「用户与账号 → 客户端策略」：

| 项 | 作用 | 缺省 |
| --- | --- | --- |
| 允许服务端转码 | 关掉只放直连 / 直接播放（省 CPU，代价是客户端兼容性变差）。关闭时 `PlaybackInfo` 直接声明不支持转码，客户端会走直连，而不是拿到一个必 403 的地址 | 开 |
| 并发转码上限 | 0 = 用进程内置上限（`EMBY_MAX_TRANSCODES` 或 CPU 核数）。设了之后超上限只**拒绝新的**转码请求（503 + 稍后再试），**不会中断正在看的人** | 0（不限） |
| 码率上限（kbps） | 客户端就算要 40Mbps 也只按上限给；同时收紧直连判定（超过上限的条目改走转码） | 0（不限） |
| 客户端黑名单 | UA 子串匹配（大小写不敏感）：命中就连播放信息都拿不到。**管理员不受限** | 空 |
| 客户端白名单 | 填了就只放行列表内的客户端（优先于黑名单）；开启后无法识别 UA 的请求也会被拒 | 空 |

- 策略落在 `SystemConfig`，**EM 与 EA 用的是同一个库**：面板上改完，出流的 EA 立刻按新策略
  放行 / 拒绝（分离部署时面板显示的本进程运行态只代表面板自身，页面上会明确提示）；
- 页面的「运行态」卡显示**本进程**正在转码几路 / 上限多少 / 由谁出流 / ffmpeg 是否可用；
- 下载开关与设备风控（每人几台设备、超限自动踢最久未用）也在这个页面上，
  逐台设备与登录日志仍在「设备与安全 / 登录日志」。
