...
「挂载」是媒体库的内容来源，后台「存储挂载」页每个类型都可以单独「测试」。扫描时来源不可用（目录不存在 / 挂载停用 / 凭据失效 / 网络不通）**不会**报错退出，而是记入 `failed_roots` 并**跳过清理阶段**——这是有意的安全网，避免把「读不到」当成「文件已删除」批量误删条目。

这类失败**不用去翻服务器日志**：媒体库卡片上会直接显示「来源不完整」和一个原因（悬停看全文），
`GET /api/admin/emby/libraries` 每个库的 `last_scan` 里也有同样的信息：

```jsonc
{
  "last_scan": {
    "status": "partial",              // running / success / partial / failed；从未扫描过为 null
    "finished_at": "2026-09-23T06:55:55",
    "duration_ms": 1234,
    "added": 3, "updated": 12, "removed": 0, "unchanged": 842,
    "removal_skipped": true,          // 来源不完整 → 本轮跳过了清理
    "failed_roots": [
...
- `success`：来源都可用且跑完了。新增 / 更新 / 删除都是这一轮的增量（不是累计值）；
- `partial`：有来源读不到。**可用的来源照常扫描**（新文件会入库），但**清理阶段被跳过**——
  磁盘上已经消失的条目会先留在库里，等来源恢复后的下一轮才删；
...
- **配置来源的直链默认允许内网**：`paths` 里的 `.strm`、存储挂载解析出的地址，即使指向
  ...
  **解析不了的域名不算内网**（离线 / 内网 DNS / 临时解析故障不会把公开 CDN 直链弄坏）。
  ...
  `EMBY_MEDIA_URL_ALLOWLIST=192.168.1.10,nas.lan` 把自己的地址放行。
...
### 后台信息架构（v2.25.0）：按交付链找页面

侧边栏不再按「技术模块」摆（服务器 / 媒体库 / 挂载 / 115 账号原本散在多个分类里），
而是按**一条交付链**分组——内容从哪来 → 谁在出流 → 用户能不能看：

| 分组 | 页面 | 一句话 |
| --- | --- | --- |
| 仪表盘 | 仪表盘 | 六张交付链数据卡（用户总数 / 当前播放 / 待处理求片 / 待处理工单 / 扫描状态 / 存储健康），每张点进明细页 |
| 用户与账号 | 用户 / 订阅与权益 / 设备与安全 / 登录日志 | 账号、会员、设备与登录 |
| 媒体与交付 | 服务器与线路 / 媒体库 / 存储来源 / 115 账号 | 谁在出流、内容从哪来、内容怎么组织 |
| 求片与内容 | 求片管理 / 公告管理 | 用户要什么、站内说什么 |
| 运营中心 | 商品与套餐 / 订单 / 兑换码 / 优惠券 / 邀请与积分 / 卡码管理 | 卖什么、钱怎么进 |
| 服务支持 | 工单 | 用户求助 |
| 系统与审计 | 系统设置 / 操作日志 / 服务健康 | 配置与事后追溯 |

两条容易找不到的入口（**页面路径与以前完全一致，旧收藏与外部链接照旧打开**）：

- **服管理**（`/admin/realms`）不再占一个一级菜单：入口在「服务器与线路」页的右上角、
  以及仪表盘「各服概况」里；
- **「多服」不再是要先理解的概念**：「当前服」与「范围（全部服 / 仅当前服）」直接放在
  「服务器与线路」页顶部，切完各页作用域跟着走（顶栏那个切换器是同一份状态）。
...
| 维度 | 按不按服 | 为什么 |
| --- | --- | --- |
| 媒体库 / 存储来源 | **按服**（挂载可留空 = 未分配） | 它们决定**哪些节点能出流、哪些用户看得到**；挂载又是主机相对资源（乙服的机器未必碰得到甲服挂的盘） |
