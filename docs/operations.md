...
- **保存「Emby 服务入口」里的分离部署后端服（EA）时**会自动拉一次 EA 视角的体检，
  不用你去逐个点「测试」（失败的挂载列表也会随之刷新）。
...
1. 后台「115 转存 → 账号配置」里点「测试 Cookie」，失效就重新粘贴（浏览器里登录 115 后复制 `uid=…; cid=…; seid=…; kid=…`）；
2. 媒体库若单独绑定了账号，确认那个配置档没有被停用或删除（停用会自动回退默认账号）；
3. 没有配置档时只能走服务器级 `PAN115_COOKIE` 兜底，确认它写在了 EM 那一侧的 `.env`（**115 任务由 EM 执行**，EA 只跑 Emby 协议面）。

任务状态同时落在数据库与 `PAN115_STATE_DIR`（默认 `EMBY_TRANSCODE_DIR/pan115`）下的 JSON 文件里，所以 EM 重启、自更新或优雅关停都不会丢进度；该目录需要可写。
