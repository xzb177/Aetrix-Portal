...
### v2.31.0 (2026-09-24) — 扫描与刮削：让「上一轮跑过的东西」真的不必再跑
- 🎞️ **配了 TMDB 的剧集库，增量扫描不再等于关掉**：集本来就不参与刮削（只有电影 / 剧集会被刮），
  但「缺 tmdb_id → 要刮」这条判断落在了所有条目的跳过判定里——于是剧中每一集每轮重扫都得完整处理，
  追新耗时几乎等于冷扫。现在只有电影 / 剧集参与刮削相关的跳过判断，安全边界一条没放松
- 🎯 **详情预取与写库同口径**：写库只在「新条目 / 带补图标记」时要详情，预取却对每个搜索命中都拉一次
  ——「重刮但信息已齐」的那次详情是白花的；现在两边按同一口径算，不再各算一套
- 🧠 **TMDB 缓存满了不再整表清空**：旧实现满了 `clear()` 会把刚预热好的结果连整张表扔掉，
  写库线程随后取同一个片名就变成一次真的网络请求（一万部片子刚好撞上两万条上限）；
  现在先清过期、再按写入顺序淘汰最旧的一批（保留 75%），读取时顺手删过期条目
- 📝 **没有外挂字幕时不再为字幕轨查一次 `max()`**：十万条目的库就是十万次多余的 SELECT
...
- 📉 **README 瘦身**：内联的 700 行版本历史与 `CHANGELOG.md` 重复（还把 README 撑到 97 KB），
  改为「最近两个版本 + 指向 CHANGELOG.md」；瘦身时留下的 `README-old.md` 快照已删掉
- 🧾 **后台写操作终于查得到**：`/api/admin/emby/*` 的成功写操作由中间件统一落进「操作日志」
  （删媒体库 / 删条目 / 删 115 账号 / 停全站转码此前查不到是谁干的）——关工单、能力测试
  两处漏网的写操作也因此被找出来补上
- 🛡️ **两条新的结构性护栏**（看的是真实路由，不是文件）：**每个管理端写端点都要留审计**、
  **每个 `/api/admin` 与 `/api/user` 写端点都要有鉴权**；例外必须显式登记并写清替代机制
  （限流 / 验签 / 验证码），名单里的死条目同样报错
